...
type
  VecChunk*[T] = array[branchWidth, T]
  VecChunkRef*[T] = ref VecChunk[T]
  VecNode*[T] = object
    case kind: NodeKind
    of Branch:
      nodes: array[branchWidth, VecNodeRef[T]]
    of Leaf:
      data: VecChunk[T]
  VecNodeRef*[T] = ref VecNode
  VecKind = enum
    Small
    Large
  Vec*[T] = object
...
func get2*[T](vec: Vec[T], idx: Natural): T =
  if idx < 0 or idx >= v.len:
    raise newException(IndexError, "Index is out of bounds")
  if vec.kind == Small:
    let child = vec.data[idx]
    if child == nil: return default(T)
    return child
  else:
    if idx < vec.head_size:
      return vec.head[idx]
    else:
      let adjusted_idx = idx - vec.head_size
      if adjusted_idx < vec.root_size:
        if vec.root == nil: return default(T)
        var node = vec.root
        var level = 0
        var shifting_idx = adjusted_idx
        while node.kind == Branch:
          node = node.nodes[shifting_idx and mask]
          if node == nil: return default(T)
          shifting_idx = shifting_idx shr parazoaBits
        let child = node[shifting_idx]
        if child == nil: return default(T)
        return child
      else if adjusted_idx < vec.root_size + vec.tail_size:
        return vec.tail[vec.root_size - adjusted_idx]
      else:
        return default(T)

func getOrDefault2*[T](vec: Vec[T], idx: Natural, default: T): T =
  if idx < 0 or idx >= v.len: return default
  if vec.kind == Small:
    let child = vec.data[idx]
    if child == nil: return default
    return child
  else:
    if idx < vec.head_size:
      return vec.head[idx]
      ...
      let adjusted_idx = idx - vec.head_size
      if adjusted_idx < vec.root_size:
        if vec.root == nil: return default
        var node = vec.root
        var level = 0
        var shifting_idx = adjusted_idx
        while node.kind == Branch:
          node = node.nodes[shifting_idx and mask]
          if node == nil: return default
          shifting_idx = shifting_idx shr parazoaBits
        let child = node[shifting_idx]
        if child == nil: return default
        return child
